...
if ($_FILES['image']['error'] == UPLOAD_ERR_NO_FILE) { // No file sent.
	...
	$dirName = $key . '.' . $extensionFile; // Set where the file will be saved
	$saveFile = move_uploaded_file($_FILES['image']['tmp_name'], "i/" . $dirName); // Prepare varible for save
	if ($saveFile) { // Execute the variable. If it work, redirect to this file.
		header('Location: view.php?img=' . $dirName);
	}
}
else { // Unkown Error
